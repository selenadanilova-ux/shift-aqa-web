...
test('Проверка URL на странице Акции', async ({page}) => {
    await page.goto('promotions')
    await expect.soft(page).toHaveURL(/.*promotions/)
})

test('Проверка текста ссылки Акции', async ({page}) => {
    await page.goto('')
...
test('Проверка URL на странице Доставка', async ({page}) => {
    await page.goto('delivery')
    await expect.soft(page).toHaveURL(/.*delivery/)
})

test('Проверка текста ссылки Доставка', async ({page}) => {
    await page.goto('')
...
test('Проверка URL на странице О нас', async ({page}) => {
    await page.goto('about')
    await expect.soft(page).toHaveURL(/.*about/)
})

test('Проверка текста ссылки О нас', async ({page}) => {
    await page.goto('')
...
test('Проверка URL на странице Контакты', async ({page}) => {
    await page.goto('contacts')
    await expect.soft(page).toHaveURL(/.*contacts/)
})

test('Проверка текста ссылки Контакты', async ({page}) => {
    await page.goto('')
...
test('Проверка URL на странице Обратная связь', async ({page}) => {
    await page.goto('feedback')
    await expect.soft(page).toHaveURL(/.*feedback/)
})

test('Проверка текста сслыки Обратная связь', async ({page}) => {
    await page.goto('')
...
test('Проверка URL на странице FAQ', async ({page}) => {
    await page.goto('faq')
    await expect.soft(page).toHaveURL(/.*faq/)
})

test('Проверка текста ссылки FAQ', async ({page}) => {
    await page.goto('')
...
test('Проверка текста кнопики Отклонить', async ({page}) => {
    await page.goto('')
    await expect.soft(page.getByTestId('cookie-consent-banner')).toBeVisible()
